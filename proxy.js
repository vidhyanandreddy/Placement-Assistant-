...
app.get("/api", async (req, res) => {
  ...
  const url =
    "https://script.google.com/macros/s/AKfycby470HdRNbyPj-cW4yxl9XkZ1wh-_jdpnaWwIh3IwIUMI6yzhMFZY7eLQ4Da7u2iE5oTw/exec" +
    (params ? `?${params}` : "");
...
app.post("/download-resumes", async (req, res) => {
  try {
    ...
    if (!resumes || !resumes.length)
      return res.status(400).json({ error: "No resumes" });
    ...
    for (const s of resumes) {
      try {
        ...
        const filename =
          (s.name || "Unknown").replace(/[/\\?%*:|"<>]/g, "_") + ".pdf";
