...
loginForm.addEventListener("submit", async (e) => {
  ...
  if (!roll || !admittedYear) {
    loginMessage.innerHTML = `<div class="text-danger">Enter roll & admitted year</div>`;
    return;
  }
  ...
  if (res?.RollNo && String(res.AdmittedYear) === admittedYear) {
    currentUser = res;
    populateStudentForm(currentUser);
    showView(viewUpload);
  } else {
    loginMessage.innerHTML = `<div class="text-danger">Invalid roll or admitted year</div>`;
  }
});
...
btnLogout.addEventListener("click", () => {
  ...
  // Clear login fields
  rollInput.value = "";
  admittedYearInputLogin.value = ""; // clear admitted year input on login
  // branchInput.value = ""; // optional, remove if branch is not used anymore
...
function renderYearBranchData(year) {
    ...
    card.innerHTML = `
      <div class="card-body p-2">
        <div class="d-flex justify-content-between mb-1">
          ...
          <button class="btn btn-sm btn-outline-primary">Download CSV</button>
        </div>
